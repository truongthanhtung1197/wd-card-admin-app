...
export const useEditAdminLogic = () => {
  ...
  const params = useParams<{ seoerId: string }>();
  const { data: adminEdit, refetch } = useGetAdminByIdQuery(params?.seoerId, {
    skip: !params?.seoerId,
  ...
  const handleGoBack = useCallback(() => {
    ...
    router.push(history[ROUTERS.MANAGEMENT_SEOER] || ROUTERS.MANAGEMENT_SEOER);
    ...
    async (values: FormValues) => {
      try {
        ...
        if (values.role) {
          ...
          formData.displayName = values.displayName;
        }
  ...
  const initialFormValues = useMemo(() => {
    return {
      ...
      displayName: adminEdit?.data?.displayName || "",
